class DropBoxController {
    constructor(){
        this.currentFolder = ['root'];//controlar o acesso aos diretorios
        ...
        this.onselectionchange = new Event('selectionchange'); //criando um evento 'nosso'
    ...
    removeTask(){
        ...
        this.getSelection().forEach(li=>{
            ...
            //como podem ser varios arquivos para excluir precisamos de varias promises
    ...
    initEvents(){
        ...
        this.btnRename.addEventListener('click',e=>{
            let li = this.getSelection()[0];//pega o primeiro elemento da lista (ja que é rename)
            let file = JSON.parse(li.dataset.file);//converte de str para obj json novamente

            let name = prompt('Renomear o arquivo:', file.name);
            if(name){
                file.name = name;

                //altera o nome no firebase se o filho especificado existir
                this.getFirebaseRef().child(li.dataset.key).set(file);
            }
        });

        //ouvinte para o evento personalisado 'selectionchange'
        ...
        this.inputFilesEl.addEventListener('change', event =>{
            this.btnSendFileEl.disabled = true; // desabilita o botao enquanto nao termina o upload

            this.uploadTask(event.target.files).then(responses=>{
                responses.forEach(resp=>{

                    //adiciona os dados na referencia capturada
    ...
    //acoes quando o upload dos arquivos terminarem
    uploadComplet(){
    ...
    getFirebaseRef(path){
        ...
        //pega a referencia do banco de dados no firebase caso exista, se nao cria uma com no 'files'
        return firebase.database().ref(path);
    }//getFirebaseRef

    connectFirebase(){
        var firebaseConfig = {
            apiKey: "AIzaSyA7ig6LpB9lRR5giJDeAKZV0zLVAINpuHc",
            authDomain: "drop-box-clone-local-sotarge.firebaseapp.com",
            databaseURL: "https://drop-box-clone-local-sotarge.firebaseio.com",
            projectId: "drop-box-clone-local-sotarge",
            storageBucket: "drop-box-clone-local-sotarge.appspot.com",
            messagingSenderId: "249825668702",
            appId: "1:249825668702:web:27147380a5ba799ef68ecf",
            measurementId: "G-VN32Y2H3BC"
        };
    ...
    ajax(url, method = 'GET', formData = new FormData(), onprogress = function(){}, onloadStart = function(){}){
        ...
        return new Promise((resolve, reject)=>{
            ...
            //pegar a proguecao dos bytes enviados ao servidor
    ...
    uploadTask(files){
        ...
        [...files].forEach(file=>{
            ...
            //API para criar um form data e mandar pelo send via ajax
            let formData = new FormData();
            formData.append('input-file', file);

            //metodo responsavel por configurar as requisicoes ajax
            promises.push(this.ajax('/upload', 'POST', formData, (event)=>{
                this.uploadProgress(event, file);
            },()=>{
                this.startUploadTime = Date.now();//captura o timestamp no momento antes de enviar os arquivos
            }));
    ...
    uploadProgress(event, file){
        ...
        let timespent = new Date() - this.startUploadTime;
        ...
        this.timeLeftEl.innerHTML = this.formaTimeToHuman(timeleft);
    }//uploadProgress
    ...
    getFileView(file,key){
        ...
        // cria um dataset para guardar a chave vinda do firebase assim na hora de editar o campo 
        //ja temos o identificador
        li.dataset.key = key; 
    ...
    readFiles(){
        ...
        //pega os dados do firebase sempre que invocado esse metodo
    ...
    initEventsLi(li){
        ...
        li.addEventListener('click', e=>{
            ...
            //tratando a selecao com o Shift
            if(e.shiftKey){
                //encontra o primeiro li selecionado
                ...
                if (firstLi){
                    ...
                    //pegar o indice do primeiro selecionado e do ultimo
                    lis.forEach((el,index)=>{
                        if(firstLi === el) indexStart = index;
                        if(li === el) indexEnd = index;
                    });

                    //ordena os indices capturados
                    ...
                    //lanca o evento 'selectionchange' para um ouvinte
